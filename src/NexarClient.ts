...
import os from 'os';
const nexarPage = (title: string, message: string) =>
  `
      <html>
    <head>
...
`;
const TOKEN_OPTIONS: https.RequestOptions = {
...
function launchBrowser(auth_request: string) {
  switch (os.platform()) {
    case 'win32':
...
class NexarClient {
  ...
  #checkTokenExp() {
    this.#exp = this.#exp || this.#accessToken.then((token) => decodeJWT(token.access_token)?.exp * 1000);

    return this.#exp.then((exp) => {
      if (exp < Date.now() + 300000) {
        //token is expired ... or will be in less than 5 minutes (300000 msec)
        this.#exp = undefined;
        this.#accessToken = this.#accessToken.then((token) => this.#refreshToken(token));
      }
